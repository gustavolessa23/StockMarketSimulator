...
public class TradingDay {
	...
	public void trade(ArrayList<Company>compList, ArrayList<Investor>invList) {
		...
		while(!CompanyGenerator.companyList.isEmpty() || !InvestorGenerator.investorList.isEmpty()) {
			...
			if(CompanyGenerator.companyList.isEmpty() || InvestorGenerator.investorList.isEmpty()) {
				...
				System.out.println("SHARES: "+CompanyGenerator.companyList.size());
				break;
			}else {
				...
				if(budget>=sharePrice) { //if budget is higher than share price (&& investor has not invested in company)
