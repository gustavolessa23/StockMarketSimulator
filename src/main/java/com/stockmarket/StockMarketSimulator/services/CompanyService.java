...
public class CompanyService {
	...
	public void populateCompanies() {
		...
		Map<Integer, Company> companyMap = new HashMap<>(); //set the key value index for the comapny list
		data.getCompanies().forEach(company ->
			companyMap.put(company.getId(), company)
				);
		data.setCompaniesMap(companyMap);
		
		companyRepository.saveAll(companies); //save the list to a database
	}
	...
	public Company getCheapestAvailableShare() {
		...
		for(int x = 0; x < data.getCompanies().size(); x++) {
			Company current = data.getCompanies().get(x); //get the first company
